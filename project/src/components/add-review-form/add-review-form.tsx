...
function AddReviewForm(): JSX.Element {
  ...
  const handleClickButton = (evt: React.SyntheticEvent): void => {
    evt.preventDefault();
      ...
      <div className="rating">
        <div className="rating__stars">
          {Array.from({length: 10}, (elem, index) => index + 1).sort((a, b) => b - a).map((elem) => {
            const index = elem;
            return <RadioStar key={index} index={index} cb={handleChangeInput}/>;
          })}
        </div>
      </div>
      <div className="add-review__text">
        <textarea onChange={handleChangeTextarea} className="add-review__textarea" name="review-text" id="review-text" placeholder="Review text" value={formData.comment} disabled={isFormBlocked} data-testid='comment'></textarea>
        <div className="add-review__submit">
          <button onClick={handleClickButton} className="add-review__btn" type="submit" disabled={!(formData.comment.length >= ReviewFormSettings.MinCommentLength && formData.comment.length <= ReviewFormSettings.MaxCommentLength) || isFormBlocked}>Post
          </button>
        </div>
      </div>
    </form>
